...
auth.post('/authorized', function (req, res) {
    ...
    logic_authorization_1.regenerateSession(req.session)
        .then(function () {
        req.AuthSvc = new logic_authorization_1.AuthSvc(req.aQuery, user, inputs, req.sessionID);
...
auth.post('/log-out', function (req, res) {
    logic_authorization_1.updateToInactiveSessionID(req.aQuery, req.session.user_uuid)
